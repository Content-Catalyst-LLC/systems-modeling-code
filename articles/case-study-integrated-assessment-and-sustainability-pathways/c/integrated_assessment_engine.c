...
int main(void) {
    ...
    printf("pathway,final_clean_energy_share,cumulative_emissions,average_climate_damages,average_transition_cost,average_land_pressure,average_water_stress,average_equity_score,final_adaptation_capacity,constraint_breach_count,average_sustainability_score\n");

    for (int i = 0; i < 6; ++i) {
        printf("%s,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%d,%.6f\n",
               summaries[i].pathway,
               summaries[i].final_clean_energy_share,
               summaries[i].cumulative_emissions,
               summaries[i].average_climate_damages,
               summaries[i].average_transition_cost,
               summaries[i].average_land_pressure,
               summaries[i].average_water_stress,
               summaries[i].average_equity_score,
               summaries[i].final_adaptation_capacity,
               summaries[i].constraint_breach_count,
               summaries[i].average_sustainability_score);
    }
