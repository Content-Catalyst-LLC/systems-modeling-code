...
if command -v gcc >/dev/null 2>&1; then
  ...
  ./outputs/stock_flow_engine > outputs/tables/c_stock_flow_engine.csv
else
...
if command -v g++ >/dev/null 2>&1; then
  echo "Running C++ workflow..."
  g++ -std=c++17 cpp/stock_flow_scenario_scanner.cpp -o outputs/stock_flow_scenario_scanner
  ./outputs/stock_flow_scenario_scanner > outputs/tables/cpp_stock_flow_scenario_scanner.csv
else
...
if command -v gfortran >/dev/null 2>&1; then
  ...
  gfortran fortran/stock_flow_recurrence_solver.f90 -o outputs/stock_flow_recurrence_solver
  ./outputs/stock_flow_recurrence_solver > outputs/tables/fortran_stock_flow_recurrence.csv
else
...
if command -v rustc >/dev/null 2>&1; then
  ...
  rustc rust/stock_flow_diagnostics_cli.rs -o outputs/stock_flow_diagnostics_cli
  ./outputs/stock_flow_diagnostics_cli
else
...
rm -f outputs/stock_flow_engine outputs/stock_flow_scenario_scanner outputs/stock_flow_recurrence_solver outputs/stock_flow_diagnostics_cli
