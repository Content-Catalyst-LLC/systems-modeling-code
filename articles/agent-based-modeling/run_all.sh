...
if command -v gcc >/dev/null 2>&1; then
  ...
  gcc c/abm_threshold_engine.c -lm -o outputs/abm_threshold_engine
  ./outputs/abm_threshold_engine > outputs/tables/c_abm_threshold_adoption.csv
else
...
if command -v g++ >/dev/null 2>&1; then
  echo "Running C++ workflow..."
  g++ -std=c++17 cpp/abm_sensitivity_scanner.cpp -o outputs/abm_sensitivity_scanner
  ./outputs/abm_sensitivity_scanner > outputs/tables/cpp_abm_sensitivity.csv
else
...
if command -v gfortran >/dev/null 2>&1; then
  ...
  gfortran fortran/abm_threshold_solver.f90 -o outputs/abm_threshold_solver
  ./outputs/abm_threshold_solver > outputs/tables/fortran_abm_threshold_adoption.csv
else
...
if command -v rustc >/dev/null 2>&1; then
  ...
  rustc rust/abm_diagnostics_cli.rs -o outputs/abm_diagnostics_cli
  ./outputs/abm_diagnostics_cli
else
