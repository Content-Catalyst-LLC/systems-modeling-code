#!/usr/bin/env bash
set -euo pipefail

ARTICLE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$ARTICLE_DIR"

mkdir -p outputs/tables outputs/figures

echo "Running Python workflow..."
python3 python/discrete_event_simulation_workflow.py

if command -v Rscript >/dev/null 2>&1; then
  echo "Running R workflow..."
  Rscript r/discrete_event_simulation_diagnostics.R
else
  echo "Rscript not found; skipping R workflow."
fi

if command -v sqlite3 >/dev/null 2>&1; then
  echo "Running SQL workflow..."
  sqlite3 outputs/tables/discrete_event_simulation.sqlite < sql/discrete_event_simulation_schema.sql
else
  echo "sqlite3 not found; skipping SQL workflow."
fi

if command -v gcc >/dev/null 2>&1; then
  echo "Running C workflow..."
  gcc c/des_queue_engine.c -lm -o outputs/des_queue_engine
  ./outputs/des_queue_engine > outputs/tables/c_des_queue_trace.csv
else
  echo "gcc not found; skipping C workflow."
fi

if command -v g++ >/dev/null 2>&1; then
  echo "Running C++ workflow..."
  g++ -std=c++17 cpp/des_sensitivity_scanner.cpp -o outputs/des_sensitivity_scanner
  ./outputs/des_sensitivity_scanner > outputs/tables/cpp_des_sensitivity.csv
else
  echo "g++ not found; skipping C++ workflow."
fi

if command -v gfortran >/dev/null 2>&1; then
  echo "Running Fortran workflow..."
  gfortran fortran/des_queue_solver.f90 -o outputs/des_queue_solver
  ./outputs/des_queue_solver > outputs/tables/fortran_des_queue.csv
else
  echo "gfortran not found; skipping Fortran workflow."
fi

if command -v go >/dev/null 2>&1; then
  echo "Running Go workflow..."
  go run go/des_scenario_runner.go
else
  echo "go not found; skipping Go workflow."
fi

if command -v rustc >/dev/null 2>&1; then
  echo "Running Rust workflow..."
  rustc rust/des_diagnostics_cli.rs -o outputs/des_diagnostics_cli
  ./outputs/des_diagnostics_cli
else
  echo "rustc not found; skipping Rust workflow."
fi

if command -v julia >/dev/null 2>&1; then
  echo "Running Julia workflow..."
  julia julia/des_queue_ensemble.jl
else
  echo "julia not found; skipping Julia workflow."
fi

rm -f outputs/des_queue_engine outputs/des_sensitivity_scanner outputs/des_queue_solver outputs/des_diagnostics_cli

echo "All available workflows complete."
